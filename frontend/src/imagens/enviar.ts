...
async function putR2(url: string, blob: Blob): Promise<void> {
  // O presign assina só ContentType e ContentLength: mandamos JPEG com o tamanho do
  // blob (o fetch põe o Content-Length sozinho). NÃO enviamos `cache-control`: esse
  // header dispararia um preflight CORS que o bucket R2 nega (só libera
  // `content-type`), fazendo o PUT falhar com "Failed to fetch".
